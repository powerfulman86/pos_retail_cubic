 ...
 */
odoo.define('pos_retail_cubic.Order', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        display_lot_popup: function () {
            ...
            if (this.pos.config.selection_existing_lots) {
                const lotsExistingOfProduct = this.pos.lot_by_product_id[selectedLine.product.id];
                let lotsWillPopup = [];
                lotsExistingOfProduct.forEach(l => {
                    lotsWillPopup.push({
                        label: l.name,
                        item: l
                    })
                })
                if (lotsExistingOfProduct.length > 0) {
                    return this.pos.gui.show_popup('selection', {
                        title: _t('Please choose 1 Lot'),
                        body: _t('Is all Lots existing for :') + selectedLine.product['name'],
                        list: lotsWillPopup,
                        confirm: function (lotSelected) {
                            lotSelected['quantity'] = selectedLine['quantity']
                            selectedLine.set_multi_lot([lotSelected]);
                        },
                        cancel: function () {
                            return _super_Order.display_lot_popup.apply(self, arguments);
                        }
                    })
                }
            }
            ...
            _super_Order.display_lot_popup.apply(this, arguments);
        },
                ...
                for (var i = 0; i < orderlines.length; i++) {
                    ...
                    if (orderline.product.tracking == 'lot') {
                        if (!orderline.lot_ids) {
                            ...
                            return this.pos.gui.show_popup('confirm', {
                                'title': _t('Error'),
                                'body': _t('Your POS config active multi Lots, it reason require set Lots for : ') + orderline.product.display_name
                            });
                        }
                        var sum = 0;
...
//                            console.log("5566699333333"+this.pos.config.show_mrp_order)
//                            if(this.pos.config.show_mrp_order){
//                                return self.pos.gui.show_popup('confirm', {
//                                    title: _t('Great Job'),
